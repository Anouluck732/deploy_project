<template>
    <div class="bg-gray-100 min-h-screen p-6">
      <div class="max-w-4xl mx-auto bg-white rounded-lg shadow-lg overflow-hidden">
        ...
        <!-- No Orders State -->
        <div v-else-if="!filteredOrders.length" class="p-6 text-center">
          <p class="font-noto-sans-lao text-gray-600">ບໍມີປະຫວັດການສັ່ງຊຶ້</p>
        </div>
  
        <!-- Orders List -->
        <div v-else>
          <div v-for="order in filteredOrders" :key="order.order_id" class="mb-8 bg-white shadow-md rounded-lg overflow-hidden">
  ...
  // Compute filtered orders to only include completed orders
  const filteredOrders = computed(() => {
    return orders.value.filter(order => order.order_status === 'complete');
  });
