<template>
        ...
        <div v-else>
          <div v-for="order in orders" :key="order.order_id" class="mb-8 bg-white shadow-md rounded-lg overflow-hidden">
            <div class="p-6 border-b border-gray-200 flex justify-between items-center bg-gray-50">
              ...
              <span :class="statusBadgeClass(order.order_status)" class="px-4 py-2 rounded-full text-sm font-medium">
                {{ order.order_status }}
              </span>
            </div>
            <div class="p-6">
              <p class="text-sm text-gray-600 mb-2">Order Date: {{ new Date(order.order_date).toLocaleDateString() }}</p>
              <p class="text-sm text-gray-600 mb-4">Shipping Address: {{ order.shipping_address }}</p>
  ...
  const statusBadgeClass = (status) => {
    switch (status) {
      case 'pending':
        return 'bg-yellow-300 text-yellow-800'; // Yellow badge
      case 'completed':
        return 'bg-green-300 text-green-800'; // Green badge
      case 'canceled':
        return 'bg-red-300 text-red-800'; // Red badge
      default:
        return 'bg-gray-300 text-gray-800'; // Default badge
    }
  };
  </script>
